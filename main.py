...
# 2. Download Dataset
print("⬇️ The dataset is being downloaded from Kagglehub...")
...
print(f"📂 Data moved to workspace: {original_dataset_dir}")
...
print("\n✂️ The dataset is being divided into Train/Val/Test (70% - 15% - 15%)...")
...
print("✅ Image Processing Engine (CLAHE + Unsharp + Augmentation) Ready.")
...
def get_safe_class_weights(train_dir):
    ...
    try:
        ...
        # 🔴 CLINICAL PRIORITY: Increase weight for Papilledema
        class_indices = temp_gen.class_indices
        if 'Papilledema' in class_indices:
             idx = class_indices['Papilledema']
             weight_dict[idx] *= 1.5
             print(f"⚠️ Clinical Adjustment: Papilledema weight increased (Index {idx}).")

        print(f"✅ Final Weights: {weight_dict}")
        return weight_dict
    except Exception as e:
        print(f"⚠️ Error calculating weights: {e}")
        return None
...
def apply_unsharp_mask(img):
    img = np.array(img, dtype=np.uint8)
    gaussian = cv2.GaussianBlur(img, (9, 9), 10.0)
    # Formül: Original + 1.5 * (Original - Blurred)
    return cv2.addWeighted(img, 1.5, gaussian, -0.5, 0, img)
...
if os.path.exists(image_dir) and len(os.listdir(image_dir)) > 0:
    ...
    print(f"📸 Seçilen Resim: {rand_img}")
    ...
    print("❌ Created. The specified folder was not found or is empty. Please check the 'image_dir' path.")
...
# Model Preprocessing Fonksiyonları
...
def merge_history(h1, h2):
    ...
    combined = {}
    # h1.history içindeki anahtarları (loss, accuracy vs.) al
...
def plot_combined_history(history_obj, model_name):
    if not hasattr(history_obj, 'history'):
        print("⚠️ The graph could not be drawn: Invalid history object.")
        return
    ...
    if len(acc) == 0:
        print("⚠️ The graph data is empty..")
        return
...
# =================================================
# 2. ROBUST TRAINING LOOP (GÜNCELLENMİŞ AUGMENTATION)
# =================================================
print("\n🚀 TRAINING PIPELINE (GEOMETRIC + PIXEL AUGMENTATION) STARTED...")
...
print("🚀 MODELLER LOADING AND ANALYZING...\n")
...
for model_name, model_path, prep_func in models_config:
    ...
    if not os.path.exists(model_path):
        ...
        if os.path.exists(backup):
            print(f"⚠️ {model_name}: ''best' was not found, 'phase1' is in use.")
            model_path = backup
        else:
            print(f"❌ {model_name}: Model file not found! Skipping.")
            continue

    print(f"⏳ {model_name} analyzing...", end=" ")
...
# ==========================================
if not all_stats:
    print("❌ models couldn't analyzed.")
else:
...
for i, (model_name, model_path, prep_func) in enumerate(models_config):
    ...
    if not os.path.exists(model_path):
        ...
        if os.path.exists(backup_path):
            print(f"⚠️ '{model_path}' not found, '{backup_path}' is in use.")
            model_path = backup_path
        else:
            print(f"❌ Model fine not dound: {model_path}. models skipped")
            continue
    ...
    try:
        test_gen = ImageDataGenerator(preprocessing_function=custom_wrapper).flow_from_directory(
            TEST_DIR, target_size=IMG_SIZE, batch_size=BATCH_SIZE,
            class_mode='categorical', shuffle=False # Shuffle False çok önemli!
        )
    except:
        print("❌ Test klasörü hatası. Yolu kontrol et.")
        continue
...
plt.tight_layout()
plt.show()
print("\n✅ All models analyzing completed!")
...
if os.path.exists(MODEL_PATH):
    ...
    if os.path.exists(target_dir):
        ...
        print(f"🔍 Analyzing File: {random_file}")
        ...
        print(f"🤖 Prediction: Class {top_pred_idx} with {confidence:.2%} confidence")
        ...
        if heatmap is not None:
            ...
            plt.tight_layout()
            plt.show()
            print("✅ Visualization Complete. Use this image for your report.")
        else:
            print("❌ Could not generate heatmap (Layer issue).")
    else:
...
def visualize_all_models_predictions(dataset_path, model_list):
    print("\n👁️ GÖRSELLEŞTİRME BAŞLIYOR (TÜM MODELLER)...")

    # --- ADIM 1: MODELLERİ GÜVENLİ YÜKLE ---
    loaded_models = {}
    print("⏳ Modeller diskten yükleniyor...")

    for name, _, _ in model_list:
        # Önce 'best' dosyasını ara
        ...
        if os.path.exists(best_path):
            ...
            print(f"  ✅ {name}: '_best' dosyası bulundu.")
        elif os.path.exists(phase1_path):
            target_path = phase1_path
            print(f"  ⚠️ {name}: '_best' yok, '_phase1' yedeği kullanılıyor.")
        else:
            print(f"  ❌ {name}: Hiçbir model dosyası bulunamadı!")
            continue
    ...
    for i in range(num_images):
        img = x_batch[i] # CLAHE uygulanmış resim

        predictions_text = ""
        true_label_idx = np.argmax(y_batch[i])
        true_label = labels[true_label_idx]

        # Sadece YÜKLENMİŞ modeller üzerinde dönüyoruz
        for name, model in loaded_models.items():
            try:
                ...
                predictions_text += f"⚠️ {name} Hatası\n"
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    gr.Markdown(
        """
        # 👁️ AI Papilledema Diagnosis System
